    ...
    const fs_path fe_path)
{
    ...
    std::string db_serial(eeprom_map["serial"].begin(), eeprom_map["serial"].end());
    ...
    return uhd::usrp::pwr_cal_mgr::make(
        ...
        [this, trx, antenna, chan_idx]() -> std::string {
            uhd::eeprom_map_t eeprom_map = get_db_eeprom();
            const std::string db_pid(eeprom_map["pid"].begin(), eeprom_map["pid"].end());
            std::ostringstream key;
            key << "x4xx_pwr_";
            try {
                key << std::hex << std::stoi(db_pid) << std::dec;
            } catch (std::invalid_argument&) {
                key << (db_pid.empty() ? "generic" : db_pid);
            }
            key << "_" << (trx == TX_DIRECTION ? "tx" : "rx");
            key << "_" << chan_idx;
            key << "_" << antenna;
            return key.str();
        },
    ...
    const fs_path fe_path)
{
    ...
    const double db_prc_rate = _rpcc->get_dboard_prc_rate();
    const double rfdc_rate   = _rpcc->get_dboard_sample_rate();
    ...
    const double lo_step_size = db_prc_rate / ZBX_RELATIVE_LO_STEP_SIZE;
    ...
    const fs_path fe_path)
{
    ...
    expert_factory::add_prop_node<double>(
        expert, subtree, fe_path / "bandwidth", ZBX_DEFAULT_BANDWIDTH);
    ...
    auto gain_profile_subscriber = [this, other_dir_gp, trx](
        ...
        if ((profile == ZBX_GAIN_PROFILE_CPLD_NOATR
            ...
            || (profile != ZBX_GAIN_PROFILE_CPLD_NOATR
                && other_dir_gp->get_gain_profile(chan) == ZBX_GAIN_PROFILE_CPLD_NOATR)) {
            RFNOC_LOG_INFO("Channel " << chan << ": Setting gain profile to `" << profile
                                      << "' for both TX and RX.");
...
void zbx_dboard_impl::_init_antenna_prop_tree(uhd::property_tree::sptr subtree,
    ...
    const fs_path fe_path)
{
    const std::string default_ant = trx == TX_DIRECTION ? ZBX_DEFAULT_TX_ANTENNA
                                                        : ZBX_DEFAULT_RX_ANTENNA;
    expert_factory::add_prop_node<std::string>(expert,
        subtree,
    ...
    const fs_path fe_path)
{
    // LO Specific
    for (auto lo : ZBX_LOS) {
        // Skip RFDC_NCO, handled after this loop
        if (lo == RFDC_NCO)
            continue;
        ...
        subtree->create<meta_range_t>(fe_path / "los" / lo / "freq/range")
            .set_publisher(
                [this, lo, chan_idx]() { return this->get_lo_freq_range(lo, chan_idx); })
            .add_coerced_subscriber([](const meta_range_t&) {
                throw uhd::runtime_error("Attempting to update freq range!");
            });
