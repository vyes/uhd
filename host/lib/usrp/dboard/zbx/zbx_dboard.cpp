 ...
 * API Calls
 *****************************************************************************/
double zbx_dboard_impl::set_rate(double requested_rate)
{
    // Zirconium does not support runtime rate changes
    const double actual_rate = _rate;
    if (not uhd::math::frequencies_are_equal(requested_rate, actual_rate)) {
...
void zbx_dboard_impl::set_tx_antenna(const std::string& ant, const size_t chan)
{
    ...
    assert_has(ZBX_TX_ANTENNAS, ant, "tx antenna");
...
void zbx_dboard_impl::set_rx_antenna(const std::string& ant, const size_t chan)
{
    ...
    assert_has(ZBX_RX_ANTENNAS, ant, "rx antenna");
...
double zbx_dboard_impl::set_tx_bandwidth(const double bandwidth, const size_t chan)
{
    ...
    if (uhd::math::fp_compare::fp_compare_epsilon<double>(bandwidth) != bw) {
...
{
    return _tree->access<double>(_get_frontend_path(TX_DIRECTION, chan) / "bandwidth")
        .get();
}
...
{
    return _tree->access<double>(_get_frontend_path(RX_DIRECTION, chan) / "bandwidth")
        .get();
}
...
double zbx_dboard_impl::get_tx_gain(const std::string& name_, const size_t chan)
{
    ...
    // The 'table' gain index. In set_tx_gain(), this instructs UHD which entry
    // from the TABLE to apply. However, in get_tx_gain(), we have no way of
    // knowing that, because any entry of the TABLE can be mapped to any DSA
    // setting. We thus either return the current DSA table index, if in the
    // no-ATR mode, or we return the last table-select value (in all other modes).
    // This will always incur a peek.
    // Note: This is a difference between using the prop tree API and the C++ API,
    // because there is no property for this gain (it wouldn't make sense for
    // most gain profiles). Since there is no property though, there is also no
    // ambiguity. In this special scenario, you just have to use the C++ API,
    // which is our recommendation anyway.
    if (name == ZBX_GAIN_STAGE_TABLE) {
        if (gain_profile == ZBX_GAIN_PROFILE_CPLD_NOATR) {
            return static_cast<double>(
                _cpld->get_current_config(chan, zbx_cpld_ctrl::atr_mode_target::DSA));
        }
        return _cpld->get_tx_table_select(chan, ATR_ADDR_TX, true);
    }
...
double zbx_dboard_impl::get_rx_gain(const std::string& name_, const size_t chan)
{
    ...
    // The 'table' gain index. In set_rx_gain(), this instructs UHD which entry
    // from the TABLE to apply. However, in get_tx_gain(), we have no way of
    // knowing that, because any entry of the TABLE can be mapped to any DSA
    // setting. We thus either return the current DSA table index, if in the
    // no-ATR mode, or we return the last table-select value (in all other modes).
    ...
    if (name == ZBX_GAIN_STAGE_TABLE) {
        ...
        return _cpld->get_rx_table_select(chan, ATR_ADDR_RX, true);
    }
...
freq_range_t zbx_dboard_impl::get_lo_freq_range(
    const std::string& name, const size_t /*chan*/)
{
