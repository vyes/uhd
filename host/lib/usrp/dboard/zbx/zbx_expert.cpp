...
void zbx_freq_fe_expert::resolve()
{
    ...
    _bandwidth = _tune_settings.max_bandwidth;
...
void zbx_tx_programming_expert::resolve()
{
    ...
    if (_profile == ZBX_GAIN_PROFILE_DEFAULT || _profile == ZBX_GAIN_PROFILE_MANUAL) {
        ...
        // TODO: Remove once ATR state changes work
        _cpld->set_tx_gain_switches(_chan, ATR_ADDR_0X, dsa_settings);
    }
    ...
    // TODO: change 0X back to tx_amp::BYPASS once ATR state changes work
    _cpld->set_tx_antenna_switches(_chan, ATR_ADDR_0X, _antenna, amp);
...
void zbx_rx_programming_expert::resolve()
{
    ...
    if (_profile == ZBX_GAIN_PROFILE_DEFAULT || _profile == ZBX_GAIN_PROFILE_MANUAL) {
        ...
        // TODO: Remove once ATR state changes work
        _cpld->set_rx_gain_switches(_chan, ATR_ADDR_0X, dsa_settings);
    }
    ...
    for (const size_t idx : ATR_ADDRS) {
        ...
        if (_antenna != "TX/RX" || idx == ATR_ADDR_RX || idx == ATR_ADDR_0X) {
...
void zbx_rx_programming_expert::_update_leds()
{
    ...
    const bool rx_on_trx = (_antenna == "TX/RX");
    // clang-format off
...
{
    if (_if2_frequency_desired.is_dirty()) {
        _rfdc_freq_desired = _if2_frequency_desired;
    }

    _rfdc_freq_coerced = _rpcc->rfdc_set_nco_freq(
        _get_trx_string(_trx), _db_idx, _chan, _rfdc_freq_desired.get());
