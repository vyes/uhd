...
is in the fpga/usrp3/top/x400/dboards/zr/cpld subdirectory, and can be looked up
in fpga/usrp3/top/x400/dboards/zr/cpld/doc/ZIRCONIUM_CPLD.htm
...
########################################################################
# TODO: Correct register map to match finalized version
REGS_TMPL = """\
########################################################################
## SLAVE SETUP
########################################################################
...
zrcpld_field_t get_field_type(const std::string& field_name) {
    % for addr in sorted(set(map(lambda r: r.get_addr(), regs))):
        % for reg in filter(lambda r: r.get_addr() == addr, regs):
    if (field_name == "${reg.get_name()}") {
        return zrcpld_field_t::${reg.get_name()};
    } else
        % endfor
    % endfor
    {
...
uint32_t get_array_size(zrcpld_field_t field) {
    switch(field) {
    % for addr in sorted(set([r.get_addr() for r in regs if r.is_array])):
        % for reg in filter(lambda r: r.get_addr() == addr, regs):
    case zrcpld_field_t::${reg.get_name()}:
...
uint32_t get_field(zrcpld_field_t field) {
    switch(field) {
    % for addr in sorted(set([r.get_addr() for r in regs if not r.is_array])):
        % for reg in filter(lambda r: r.get_addr() == addr, regs):
    case zrcpld_field_t::${reg.get_name()}:
...
uint32_t get_field(zrcpld_field_t field, const size_t idx) {
    switch(field) {
    % for addr in sorted(set([r.get_addr() for r in regs if r.is_array])):
        % for reg in filter(lambda r: r.get_addr() == addr, regs):
    case zrcpld_field_t::${reg.get_name()}:
...
void set_field(zrcpld_field_t field, uint32_t value) {
    switch(field) {
    % for addr in sorted(set([r.get_addr() for r in regs if not r.is_array])):
        % for reg in filter(lambda r: r.get_addr() == addr, regs):
    case zrcpld_field_t::${reg.get_name()}:
...
void set_field(zrcpld_field_t field, uint32_t value, const size_t idx)
{
    switch(field) {
    % for addr in sorted(set([r.get_addr() for r in regs if r.is_array])):
        % for reg in filter(lambda r: r.get_addr() == addr, regs):
...
uint16_t get_addr(zrcpld_field_t field) {
    switch(field) {
    % for addr in sorted(set([r.get_addr() for r in regs if r.is_array])):
        % for reg in filter(lambda r: r.get_addr() == addr, regs):
    case zrcpld_field_t::${reg.get_name()}:
