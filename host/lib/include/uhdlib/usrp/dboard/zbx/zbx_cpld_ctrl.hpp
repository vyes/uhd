...
public:
    ...
    /*! Return the current TX table-select value
     *
     * Note that this doesn't necessarily allow inferences on the gain settings.
     * The DSA table could have manually been altered after the table select was
     * triggered.
     *
     * \param Table index.
     */
    uint8_t get_tx_table_select(
        const size_t channel, const uint8_t idx, const bool update_cache);

    /*! Return the current RX table-select value
     *
     * Note that this doesn't necessarily allow inferences on the gain settings.
     * The DSA table could have manually been altered after the table select was
     * triggered.
...
private:
     ...
     */
    void update_field(const zr_cpld_regs_t::zrcpld_field_t field, const size_t idx);
    ...
    zr_cpld_regs_t _regs = zr_cpld_regs_t();
