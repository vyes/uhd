...
/*! \brief Provide access to a Zirconium radio.
 */
...
public:
    ...
    {
        const double bw =
            _tree->access<double>(_get_frontend_path(TX_DIRECTION, chan) / "bandwidth")
                .get();
        return uhd::meta_range_t(bw, bw);
    }
    uhd::meta_range_t get_rx_bandwidth_range(size_t chan) const override
    {
        const double bw =
            _tree->access<double>(_get_frontend_path(RX_DIRECTION, chan) / "bandwidth")
                .get();
        return uhd::meta_range_t(bw, bw);
    }
    ...
    freq_range_t get_lo_freq_range(const std::string& name, const size_t chan);
    ...
    // LO Test Mode Enabled Control
    void set_tx_lo_test_mode_enabled(
        const bool enabled, const std::string& name, const size_t chan);
    void set_rx_lo_test_mode_enabled(
        const bool enabled, const std::string& name, const size_t chan);
    bool get_tx_lo_test_mode_enabled(const std::string& name, const size_t chan);
    bool get_rx_lo_test_mode_enabled(const std::string& name, const size_t chan);
