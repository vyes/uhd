...
constexpr size_t RPC_TIMEOUT = 10; // ms
...
// These parameters are specific to each other *and* to the mock x4xx data
// in rc/cal/x4xx_pwr_mock_tx.cal.  If one changes, they will likely all have to
// change.
static constexpr double tx_given_gain                                      = 30;
static constexpr double tx_given_power                                     = -30;
static const std::vector<std::array<double, 2>> tx_power_cal_mock_expected = {
    {-10, 6e+08},
    {-11, 1e+09},
    {-12, 2e+09},
    {-18, 3e+09},
...
BOOST_FIXTURE_TEST_CASE(zbx_tx_power_api, x400_radio_fixture)
{
    ...
    for (size_t chan = 0; chan < ZBX_NUM_CHANS; chan++) {
        ...
        BOOST_REQUIRE(gain_coerced == tx_given_gain);
        for (auto iter = tx_power_cal_mock_expected.begin();
             iter != tx_power_cal_mock_expected.end();
             iter++) {
            // Setting a power reference should kick us into power tracking mode
            test_radio->set_tx_power_reference(tx_given_power, chan);

            test_radio->set_tx_frequency(iter->at(1), chan);
            // If the tracking mode is properly set, we should match our expected criteria
            // for power reference levels
            double pow_ref = std::floor(test_radio->get_tx_power_reference(chan));
            BOOST_REQUIRE(pow_ref == iter->at(0));
...
// These parameters are specific to each other *and* to the mock x4xx data
// in rc/cal/x4xx_pwr_mock_rx.cal.  If one changes, they will likely all have to
// change.
static const double rx_given_gain                                          = 30;
static const double rx_given_power                                         = -30;
static const std::vector<std::array<double, 3>> rx_power_cal_mock_expected = {
    {-28, 6e+08},
    {-30, 1e+09},
    {-30, 2e+09},
    {-28, 3e+09},
...
BOOST_FIXTURE_TEST_CASE(zbx_rx_power_api, x400_radio_fixture)
{
    ...
    for (size_t chan = 0; chan < ZBX_NUM_CHANS; chan++) {
        ...
        BOOST_REQUIRE(gain_coerced == tx_given_gain);
        for (auto iter = rx_power_cal_mock_expected.begin();
             iter != rx_power_cal_mock_expected.end();
             iter++) {
            // Setting a power reference should kick us into power tracking mode
            test_radio->set_rx_power_reference(rx_given_power, chan);

            test_radio->set_rx_frequency(iter->at(1), chan);
            // If the tracking mode is properly set, we should match our expected criteria
            // for power reference levels
            double pow_ref = std::floor(test_radio->get_rx_power_reference(chan));
            BOOST_REQUIRE(pow_ref == iter->at(0));
...
BOOST_FIXTURE_TEST_CASE(zbx_rx_gain_profile_test, x400_radio_fixture)
{
    ...
    BOOST_CHECK_EQUAL(23.0, test_radio->get_rx_gain("TABLE", 0));
...
BOOST_FIXTURE_TEST_CASE(zbx_tx_gain_profile_test, x400_radio_fixture)
{
    ...
    BOOST_CHECK_EQUAL(23.0, test_radio->get_tx_gain("TABLE", 0));
    // This will update RX and XX registers (that's the difference to table_noatr)
