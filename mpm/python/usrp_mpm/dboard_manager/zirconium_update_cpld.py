...
def find_offset(dboard):
    ...
    if dboard == 0:
        return DAUGHTERBOARD0_OFFSET
    elif dboard == 1:
        return DAUGHTERBOARD1_OFFSET
...
def do_update_cpld(filename, daughterboards, updater_mode):
    ...
    """
    logger = get_logger('update_cpld')
    logger.info("Programming CPLD of dboards {} with image {} using {} mode".format(daughterboards, filename, updater_mode))
    ...
    if updater_mode == 'legacy':
        ...
        logger.error("Invalid updater method {} given".format(updater_mode))
        return False
...
def main():
    ...
    def parse_args():
        ...
        parser = argparse.ArgumentParser(description='Update the CPLD image on NI ZBX')
        parser.add_argument("--file", help="Filename of CPLD image",
                            default="/lib/firmware/ni/cpld-zirconium.rpd")
