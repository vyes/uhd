    ...
    def poke8(self, addr, val, overwrite_mask=False):
        ...
        if overwrite_mask == False:
            ...
            if mask is not None:
                ...
                val = val & ~mask
                val = val | current_val
                self.log.trace("Attention: writing to register {:02x} with masked bits, mask 0x{:02x} was applied, resulting in value {:02x}".format(addr, mask, val))
