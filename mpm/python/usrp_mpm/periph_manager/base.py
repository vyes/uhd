...
class PeriphManagerBase(object):
    ...
    def _add_public_methods(self, src, prefix=""):
        """
        Add public methods (=API) of src to dest object. To avoid naming
        conflicts and make relation clear all added methods are prefixed
        ...
        for name in [name for name in dir(src)
            ...
            if hasattr(self, name):
                self.log.warn("Cannot add method {} because it would "
                              "overwrite existing method.".format(name))
            else:
                method = getattr(src, name)
                self.log.trace("Add function % s", name)
                setattr(self, prefix + name, method)
