...
class TestEeprom(TestBase):
    ...
    tagmap = {
        # typical board map
        0x42: usrp_mpm.tlv_eeprom.NamedStruct('< H H H 7s 1x',
                                     ['pid', 'rev', 'rev_compat', 'serial']),
        # tow additional maps to test unpack of multiple maps in a single eeprom
        0x47: usrp_mpm.tlv_eeprom.NamedStruct('< B B H',
                                     ['byte1', 'byte2', 'short']),
        0x11: usrp_mpm.tlv_eeprom.NamedStruct('< L',
                                     ['long'])
        }
    magic = 0xCEF10F00
    ...
    def _check_data(self, data, **kwargs):
        self.assertEqual(len(kwargs), len(data),
                         "test does not check all items (Expected: %s Read: %s" % (kwargs.keys(), data.keys()))
        for key, value in kwargs.items():
            self.assertTrue(key in data, "data does not contain %s" % key)
            self.assertEqual(data[key], value, "%s does not match. "
                    "Expected: %s, Received: %s" % (key, value, data[key]))

    def _get_eeprom_filename(self, name):
    ...
    def test_eeprom_legacy_minsize(self):
        ...
        self.assertRaises(struct.error,
                          ...
                          self._get_eeprom_filename("empty.eeprom"),
                          0,
    ...
    def test_eeprom_mboard_v1(self):
        ...
        (data, raw_data) = usrp_mpm.eeprom.read_eeprom(
            self._get_eeprom_filename("legacy_mboard_v1.eeprom"),
            0,
            MboardEEPROM.eeprom_header_format,
            keys,
        ...
        self._check_data(
            ...
            rev = 0xABCD,
            serial = b"mbrd_v1",
            mac_addresses = b'\x01\x02\x03\x04\x05\x06\x00\x00\x0f\x0e\r\x0c\x0b\n\x00\x00\x11"3DUf\x00\x00',
            CRC = 0x02CC50AA)

    def test_eeprom_mboard_v2(self):
        # read valid v2 map
        (data, raw_data) = usrp_mpm.eeprom.read_eeprom(
            self._get_eeprom_filename("legacy_mboard_v2.eeprom"),
            0,
            MboardEEPROM.eeprom_header_format,
            MboardEEPROM.eeprom_header_keys,
            PeriphManagerBase.mboard_eeprom_magic)
    ...
    def test_eeprom_mboard_v3(self):
        # read valid v3 map
        (data, raw_data) = usrp_mpm.eeprom.read_eeprom(
            self._get_eeprom_filename("legacy_mboard_v3.eeprom"),
            0,
            MboardEEPROM.eeprom_header_format,
            MboardEEPROM.eeprom_header_keys,
            PeriphManagerBase.mboard_eeprom_magic)
    ...
    def test_eeprom_tlv_single(self):
        ...
        (data, rawdata) = usrp_mpm.tlv_eeprom.read_eeprom(
            self._get_eeprom_filename("tlv_single.eeprom"),
            self.tagmap,
            self.magic)
        self._check_data(
            data,
    ...
    def test_eeprom_tlv_multiple(self):
        ...
        (data, rawdata) = usrp_mpm.tlv_eeprom.read_eeprom(
            self._get_eeprom_filename("tlv_multiple.eeprom"),
            self.tagmap,
            self.magic)
        self._check_data(
            data,
    ...
    def test_eeprom_tlv_unknown_tagmap(self):
        ...
        # of runtime error
        (data, rawdata) = usrp_mpm.tlv_eeprom.read_eeprom(
            self._get_eeprom_filename("tlv_unknown_tagmap.eeprom"),
            self.tagmap,
            self.magic)
    ...
    def test_eeprom_tlv_unknown_wrong_maplen(self):
        ...
        self.assertRaises(TypeError,
                          ...
                          self._get_eeprom_filename("tlv_wrong_maplen.eeprom"),
                          self.tagmap,
                          self.magic)
