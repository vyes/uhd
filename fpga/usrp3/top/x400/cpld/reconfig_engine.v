...
// Description: This file implements the registers and the state machine to
// interface with Intel's IP for the Max 10 FPGA that allows in-field updates
// to the primary FPGA image. This state machine has been designed to provide
// a level of abstraction between the register interface provided to user and
// the details of interfacing with Intel's On-Chip Flash IP block. The user
...
// verify a new primary FPGA image. Since the purpose of this file is to allow
// modification to an FPGA image care has been taken to mitigate data corruption.
//
// The interface to Intel's On-Chip Flash IP block implemented in this file is
// based on the information found in the Max 10 User Flash Memory User Guide
...
module reconfig_engine #(
  ...
  // memory initialization enabled
  // 0 if MAX10 internal configuration set to single compressed image
  // 1 if MAX10 internal configuration set to single compressed image with memory initialization
    ...
    // read request
      ...
      case (s_ctrlport_req_addr)
        ...
        BASE_ADDRESS + FLASH_CFM0_START_ADDR_REG: begin
          s_ctrlport_resp_data <= MEM_INIT ? FLASH_PRIMARY_IMAGE_START_ADDR_MEM_INIT : FLASH_PRIMARY_IMAGE_START_ADDR;
        end
...
// The Intel on-chip flash control interface that has two registers...a
// Status Register at address 0 and a Control Register at address 1. The
// constants defined below indentify fields and values of interest in each
// register. These are taken directly from the Max 10 Flash Memory User Guide.
...
always @(posedge ctrlport_clk) begin
  ...
  // rising edge clock
    ...
    case(state)
      ...
      // Transition from WP_DISABLED when write protection is enabled
      // or when write/erase operations are initiated. A few things to note...
      //  * Enabling write protection takes priority, regardless of what other
      //    control bits may be asserted simultaneously, followed by writes, and
      //    lastly erases.
      //  * The user should not strobe both the *write_stb and *erase_stb bits
      //    simultanesously, but if they do the state machine returns to IDLE
      //    (thereby enabling write protection) and the *write_err and
      ...
      //    First write protection must be disabled, and only then can the flash
      ...
      WP_DISABLED: begin
        ...
        if (flash_enable_wp_stb ||
          (flash_erase_stb && flash_write_stb)) begin
          state <= IDLE;
          csr_write <= 1'b1;
          csr_addr <= CONTROL_REG_ADDR;
      ...
      // WAIT_FOR_READ_DATA_VALID until the flash data interface data_readdatavalid
      // signal asserts, indicating the data is now valid. Intel's
      // documentation does not provide guidance on the expected time for
      // data_readdatavalid to assert. From simulation, however, data_readdatavalid
      // asserts four Clk cycles after the Read pulse ends.
      // The data_readdatavalid signal from the flash data interface pulses for one
      // clock cycle. Only during this pulse is the data valid.
      ...
      // control interface Status Register. To verify that the read operation is
      // complete the StatusRegister is polled until the 'busy' field indicates
      // the flash is idle. This polling operation is implemented with
      // CHECK_READ_STATUS below. GET_READ_STATUS exists
      // to set the address of the flash control interface to the Status
      // Register and pulse the read bit of the flash control interface.
      // CHECK_READ_STATUS evaluates the resulting Status Register
      // data and steer the state machine accordingly. (see Figure 6 in
      // Intel's Max 10 User Flash Memory User Guide
      // for a waveform on this request and check meachism)
      // Successful read operations result in the state machine returning to IDLE. Failing
      // read operations assert the flash_read_err bit before returning to IDLE.
      // From simulation, the 'busy' field returns to IDLE on the third read.
      ...
      // Write Flash
      //---------------
      // Per Intel's Max 10 User Flash Memory User Guide, the Write bit of the
      // flash data interface should be asserted while maintaining address and
      // data until the flash interface de-asserts the data_waitrequest bit.
      // Transition from WP_DISABLED to WAIT_FOR_WRITE_COMPLETE causes the
      // write bit to assert and address and data to be set. The state machine
      // remains in this state until the data_waitrequest bit de-asserts. Per
      // Intel's Max 10 User Flash Memory User Guide, the data_waitrequest signal is
      // expected to de-assert within 555 usec.
      ...
      //   * Upon successful completion of a write operation the state machine
      //     returns to WP_DISABLED. This allows repeated writes of new data
      //     without having to disable/enable for each write.
      //   * When a failure is detected the state machine transitions to
      //     to IDLE thereby enabling write
      //     protection. Failure of a write is not expected. Write protection is
      //     enabled in the event of a failure to mitigate further data
      //     corruption.
      ...
      // Erase Flash
      //-------------
      // Erasing the primary configuration image requires a single write to the
      // flash control interface Control Register. Only one sector needs to be
      // erased to erase the entire primary configuration image. Transition from
      // WP_DISABLED to ERASE_SECTOR causes data to be written to the Control
      // Register to erase this sector and pulse the flash control interface
      // write bit.
      ERASE_SECTOR: begin
      ...
      // 'busy' field of the flash control interface Status Register indicating
      // the erase operation is in progress. After initiating the erase
      // operation, GET_ERASE_BUSY and CHECK_ERASE_BUSY implement a polling
      // operation to determine when the erase operation has started.
      ...
      // described above. Intel's documentation indicates that erase operations
      // take a maximum of 350 msec.
