...
module rf_core_100m (
  ...
  // Data comes from the RFDC... as 2 SPC, separate streams for each channel and I/Q.
  ...
  // ADC Data from the RFDC arrives here as 2 SPC with separate I and Q streams.
  // It leaves the adc_100m_bd as 1 SPC with I and Q packed into a single 32 bit word.
  genvar adc_num;
  generate
  for (adc_num=0; adc_num < (NUM_ADC_CHANNELS); adc_num = adc_num + 1)
    begin : adc_gen
      //vhook   adc_100m_bd adc_100m_bd_gen
      ...
      adc_100m_bd
        adc_100m_bd_gen (
          ...
          .adc_data_out_tdata        (adc_data_out_tdata[adc_num]), //out STD_LOGIC_VECTOR(31:0)
          ...
          .adc_i_data_in_tdata       (adc_data_in_i_tdata[adc_num]), //in  STD_LOGIC_VECTOR(31:0)
          ...
          .adc_q_data_in_tdata       (adc_data_in_q_tdata[adc_num]));//in  STD_LOGIC_VECTOR(31:0)
      //vhook_nowarn rf_core_100m.fir_resetn_rclk2
      //vhook_nowarn rf_core_100m.adc_data_out_resetn_dclk
      //vhook_nowarn rf_core_100m.adc_data_out_t*
      //vhook_nowarn rf_core_100m.adc_tile_dout_*_t*
      //vhook_nowarn rf_core_100m.invert_adc_iq_rclk2
    end
  endgenerate
  ...
  // These streams are then interpolated by dac_100m_bd, and form a single stream per
  // channel, 2 SPC, packed: msb [Sample1Q, Sample1I, Sample0Q, Sample0I] lsb
  generate
  for (dac_num=0; dac_num < (NUM_DAC_CHANNELS); dac_num = dac_num + 1)
    begin : dac_gen
      //vhook   dac_100m_bd dac_100m_bd_gen
      ...
      dac_100m_bd
        dac_100m_bd_gen (
          ...
          .rfdc_clk                   (rfdc_clk),                    //in  STD_LOGIC
          .rfdc_clk_2x                (rfdc_clk_2x),                 //in  STD_LOGIC
          .dac_data_out_tdata         (dac_data_out_tdata[dac_num]), //out STD_LOGIC_VECTOR(63:0)
          ...
          .dac_data_in_tdata          (dac_data_in_tdata[dac_num]),  //in  STD_LOGIC_VECTOR(31:0)
          .dac_data_in_tvalid         (dac_data_in_tvalid[dac_num]), //in  STD_LOGIC
          .dac_data_in_tready         (dac_data_in_tready[dac_num]));//out STD_LOGIC

      //vhook_nowarn rf_core_100m.dac_tile_din_t*
    end
  endgenerate

  // Data is released to the RFDC... as 2 SPC, separate streams per channel (I/Q together)
